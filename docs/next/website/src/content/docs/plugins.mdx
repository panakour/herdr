...
receive `HERDR_PLUGIN_ACTION_ID`, and actions started from a keybinding receive
the invoking client's `HERDR_CLIENT_ID`; startup and event hooks receive
...
agent, selected text, clicked URL, link handler, and `client_id` fields when
they are available for that invocation. Shell plugins can read the individual env vars
