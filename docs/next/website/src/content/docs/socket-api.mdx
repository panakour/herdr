...
| Client | `client.window_title.set`, `client.window_title.clear`, `client.session.switch` |
...
Move one attached client to another local named session in the same terminal:

```json
{"id":"req_switch","method":"client.session.switch","params":{"session":"work","client_id":3,"cwd":"/srv/work"}}
```

`client_id` is optional and defaults to the current foreground client. Custom
commands and plugin actions started from a keybinding receive the invoking
client's id as `HERDR_CLIENT_ID`. The server only asks; the client prepares an
inactive connection to the target, starting its server when necessary, then
detaches from the source after the target accepts the handshake. Preparation
runs in the background; startup or handshake failures leave the source usable. It
keeps every other attached client where it is. The optional `cwd` seeds the
first workspace of a server started by this switch and is ignored when the
session already has workspaces. The response is
`type: "client_session_switch"` with `accepted`, `session`, `client_id`, and
reason `requested` or `no_foreground_client`. `accepted: true` means the server
queued the request for the client, not that the client completed the switch.
Requests for the current session
fail with `same_session`; unknown, detached, or direct terminal clients fail
with `client_not_found` or `client_not_switchable`. Clients from before this
release ignore the request. Remote clients connected with `herdr --remote`
