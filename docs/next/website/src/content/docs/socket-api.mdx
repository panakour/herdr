...
client's id as `HERDR_CLIENT_ID`. The server only asks; the client detaches,
starts the target session's server when it is not running, reconnects, and
...
`type: "client_session_switch"` with `switched`, `session`, `client_id`, and
reason `requested` or `no_foreground_client`. Requests for the current session
